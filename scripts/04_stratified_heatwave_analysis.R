## ---------------------------
## Script name: Acute and prolonged heatwave analyses among stratified study population
## Author: Grace Kuiper
## Version: 10/11/2021
...
##          among study population stratified by age, sex, and race.
...
heatwave_days <- heatwave_days %>%
  pivot_wider(names_from="threshold",values_from=c("previous_heatwave","past_five_days",
  ...
  if (nrow(casecross_list[[o]])>0) {
    for(i in 1:length(thresholds)) {
      ...
      df_temp <- df %>%
        ...
        rename(past_five_days_relevant=!!paste0("past_five_days_",threshold)) %>% #acute heatwave
        rename(heatwave_day_relevant=!!paste0("heatwave_day_",threshold)) %>%
...
file <- paste0("../results/stratified/heatwave/",
               strata,"/",group,"/total_heatwave_analysis.xlsx")
wb <- createWorkbook()
